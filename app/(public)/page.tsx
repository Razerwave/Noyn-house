  ...
  return (
    <>
      <section className="hero">
        <Image className="hero-image" src="/images/hero-house.webp" alt="" fill sizes="100vw" preload unoptimized />
      ...
      {featuredHouses.length > 0 && <section className="section soft"><div className="container"><div className="section-head"><div><div className="eyebrow">Онцлох загварууд</div><h2 className="section-title">Танд тохирох орон зай</h2></div><Link className="text-link" href="/houses">Бүх загвар үзэх <ArrowRight size={16} /></Link></div><div className="cards">
        {featuredHouses.map(model => <Link href={`/houses/${model.slug}`} prefetch={true} className="card" key={model.id ?? model.slug}><div className="card-image"><Image src={model.image} alt={`${model.name} хаус`} fill sizes="(max-width:680px) 100vw, 33vw" /><span className="badge">{model.category}</span></div><div className="card-body"><h3>{model.name}</h3><p>{model.description}</p><div className="spec-row"><span><Ruler size={14}/>{model.area}</span><span><Building2 size={14}/>{model.floors} давхар</span><span><BedDouble size={14}/>{model.bedrooms} унтлагын өрөө</span></div><span className="text-link">Дэлгэрэнгүй үзэх <ArrowRight size={15}/></span></div></Link>)}
      </div></div></section>}
      <MaterialSection />
      ...
      {latestProjects.length > 0 && <section className="section soft"><div className="container"><div className="section-head"><div><div className="eyebrow">Хийсэн төслүүд</div><h2 className="section-title">Бодитоор бүтсэн орон зай</h2></div><Link className="text-link" href="/projects">Бүх төсөл үзэх <ArrowRight size={16}/></Link></div><div className="cards">{latestProjects.map(project=><Link href={`/projects/${project.slug}`} prefetch={true} className="card" key={project.id ?? project.slug}><div className="card-image"><Image src={project.image} alt={project.title} fill sizes="(max-width:680px) 100vw, 33vw" /></div><div className="card-body"><h3>{project.title}</h3><p>{project.location} · {project.area} · {project.year}</p><span className="text-link">Төслийн дэлгэрэнгүй <ArrowRight size={15}/></span></div></Link>)}</div></div></section>}
      {latestArticles.length > 0 && <section className="section home-news-section"><div className="container"><div className="section-head"><div><div className="eyebrow">Мэдлэгийн сан</div><h2 className="section-title">Мэдээ, зөвлөгөө</h2></div><Link className="text-link" href="/news">Бүх нийтлэл үзэх <ArrowRight size={16}/></Link></div><div className="cards">{latestArticles.map(article => <Link href={`/news/${article.slug}`} prefetch={true} className="card" key={article.id ?? article.slug}><div className="card-image"><Image src={article.image} alt={article.title} fill sizes="(max-width:680px) 100vw, 33vw" /><span className="badge">{article.category}</span></div><div className="card-body"><h3>{article.title}</h3><p>{article.summary}</p><span className="text-link">Унших <ArrowRight size={15}/></span></div></Link>)}</div></div></section>}
      <section className="section home-process-section"><div className="container"><div className="section-head"><div><div className="eyebrow">{processContent.homeEyebrow}</div><h2 className="section-title">{processContent.homeTitle}</h2></div><Link className="text-link" href="/process">Дэлгэрэнгүй явц <ArrowRight size={16}/></Link></div><div className="process-grid">{processContent.steps.slice(0,6).map((step,i)=><div className="process-item" key={step.id ?? `${i}-${step.title}`}><b>{String(i+1).padStart(2,'0')}</b><h3>{step.title}</h3><p>{step.description}</p></div>)}</div></div></section>
