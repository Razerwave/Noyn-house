...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      {latestArticles.length > 0 && <section className="section home-news-section"><div className="container"><div className="section-head"><div><div className="eyebrow">Мэдлэгийн сан</div><h2 className="section-title">Мэдээ, зөвлөгөө</h2></div><Link className="text-link" href="/news">Бүх нийтлэл үзэх <ArrowRight size={16}/></Link></div><div className="cards">{latestArticles.map(article => <Link href={`/news/${article.slug}`} className="card" key={article.id ?? article.slug}><div className="card-image"><Image src={article.image} alt={article.title} fill sizes="(max-width:680px) 100vw, 33vw" /><span className="badge">{article.category}</span></div><div className="card-body"><h3>{article.title}</h3><p>{article.summary}</p><span className="text-link">Унших <ArrowRight size={15}/></span></div></Link>)}</div></div></section>}
