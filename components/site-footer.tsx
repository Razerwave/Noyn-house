  ...
  return (
    <footer className="footer">
      <div className="container footer-grid">
        <div><Logo inverse /><p>{settings.siteDescription}</p></div>
        ...
        <div><h4>Холбоо барих</h4><a href={`tel:${phoneHref}`}><Phone size={15} /> {settings.phone}</a>{settings.phone2 && <a href={`tel:${phone2Href}`}><Phone size={15} /> {settings.phone2}</a>}<a href={`mailto:${settings.email}`}><Mail size={15} /> {settings.email}</a><span><MapPin size={15} /> {settings.address}</span></div>
